...
def main():
    ...
    #
    distances = [ range_distance_min + (range_distance_max - range_distance_min) * index / range_steps for index in range(range_steps+1) ]
    frequencies = [ range_frequency_max - (range_frequency_max - range_frequency_min) * index / range_steps for index in range(range_steps+1) ]
    ...
    while not rospy.is_shutdown():
        ...
        if ( len(buf) - index_buf ) < buffer_size:
            ...
            time_played += 1.0 * ( len(buf) - index_buf ) / ( msg_info.width * msg_info.sampling_rate )
        else:
        ...
        rospy.loginfo( "distance: {}, freq: {}".format( distance, frequencies[index_distance] ) )
